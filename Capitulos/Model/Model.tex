...
and with a high separation of pseudorapidity are labeled as candidates of processes of VBF. The Feynman diagrams illustrating the processes already described is shown in Figure \ref{Signal_feynman}.

%CORREGIR IMAGENEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEES

\begin{figure}
\label{Signal_feynman}
\centering
\begin{minipage}{.6\textwidth}
  \centering
  \includegraphics[width=.7\linewidth]{./Capitulos/Model/hadron_signal}
  \caption{Feynman diagram for the hadron signal}
  \label{signal_hadron_feynman}
\end{minipage}%
\begin{minipage}{.6\textwidth}
  \centering
  \includegraphics[width=.7\linewidth]{./Capitulos/Model/leptonic_signal}
...
$\vec{E_T^{miss}}$, and one lepton. Thus, the principal backgrounds of the signal that have a similar final state are the W+jets background and the DY+jets background. In a inferior magnitude there
is other background that comes from the quark-antiquark top annihilation.

 
\subsection{W+Jets Background}
  
The events in which is produced a boson W with jets has a large probabibity to occur in the collisions at the LHC. The W decay determines the final state of this event. Most of the times the W boson
have a hadronic decay and the other times have a leptonic decay. In the last type of decayment, the boson W desintegrates into a lepton and a neutrino. Sometimes, in these events the particles 
...
in Figure \ref{w_jets_feynman}.
  ...
  %Arreglar numero figuraaaaaaaaaaaaaaaaaaaaaa
 ...
 produce a virtual photon or a Z boson. The concept virtual photon means that this particle is created for a really short period of time. We studied this process only when the Z boson decays into a 
 ...
 tau and antitau ($\tau \overline{\tau}$). This proccess was simulated with the presence of a jet from ISR. Thus, the final state is conformed by one tau, jets (from the ISR procces), 

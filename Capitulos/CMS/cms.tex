...
move towards the cathode. The CSC have fast response time, fine sementation and are radiation resistant. Thus they are able to take precise measurements time and position. These detectors cover the
...
conformed by two plates, one is the cathode and the other the anode and these plates are separated by a gas. When a charge particle crosses the detector, the resulting free electrons move to the anode
The electric signal is by metallic strips after a precise dime delay. The set of hit strips gives a precise measurement of the muon position and momentum.
...
to store or not store an event in order to reduce the computational resources. Triggers have the task of apply a primary selection on the data at real-time. The triggers take the data and quickly 
decide which events that are interesting pass to the next phase of filterig. The triggers need to have a rejection factor of almost $10^7$. As a consequence, it allows to store just around 100-200 
carefully selected events per second to be proccess later. 
...
It reduces the data rate to nearly 100 kHz. Hight level triggers use a network of thousands processors and fast switches. They get the information gradually and use algorithms in order to reduce 
the ammount of data. The final product is that at a rate of 150-200 Hz each event occupies around 1.5 Mb. The former leads to the LHC to have an annual data volume of around 10 PB . After an event 
...
processors with several thousands of central processing units. The event then is directed to a single processor with the objective of perform detailed calculations of the critical parameters of the 
event and reduce the ammount of data.
